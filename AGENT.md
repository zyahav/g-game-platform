...
7. If `.student-package/` exists in the project root after generation, copy:
   `.student-package/publish.toml` -> `publish.toml`, `.student-package/kaya.env` -> `kaya.env` if present, and `.student-package/keys/` -> `keys/`.
8. Run the generated project's environment preflight and automatic repair path before asking the student for setup help.
9. Stop working inside this platform clone once generation is complete and continue in the generated project directory.
